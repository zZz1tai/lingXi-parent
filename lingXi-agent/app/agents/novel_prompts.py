...
## 输出规范
- 用户要求「续写下一章」「生成一章」或「生成完整章节」时，直接输出完整
  章节正文，不加前言、解释、章节号或作者按语；正文目标约 2500 字，建议
  控制在 2300～2700 字，不得只给片段。
- 其他「续写一段」或普通正文创作默认 800～1500 字；润色、改写任务保持
  原文篇幅与任务范围。用户明确指定长度时，以用户要求为准。
